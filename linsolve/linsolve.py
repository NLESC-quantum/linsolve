...
class LinearSolver:
    ...
    def _get_AtA_Aty_sparse(self, xs_ys_vals, y):
        ...
        for i in range(AtA.shape[1]):
            ...
            for j in range(i, AtA.shape[1]):
                AtA[:, i, j] = sum(
                    [A[i][x].conj() * A[j][x] for x in A[i] if x in A[j]]
                )
    ...
    def _invert_qubols(self, A, y, rcond):
        ...
        for m, y in zip(AtA, Aty):
            ...
            qubols = QUBOLS(m_, y_)
            solution_vector = qubols.solve(
                nqbit=self.qubo_num_qbits, num_reads=self.qubo_num_reads
            )
    ...
    def _invert_vqls(self, A, y, rcond):
        ...
        for m, y in zip(AtA, Aty):
            np.savetxt("matrix_8linear.txt", m)
            np.savetxt("rhs_8linear.txt", y)
            exit()
    ...
    def _invert_vqls_runtime(self, A, y, rcond):
        ...
        with Session(service=service, backend=self.ibmq_backend) as session:
            ...
            for m, y in zip(AtA, Aty):
                ...
                solution_vector = self.post_process_vqls_solution(
                    m, y, np.real(Statevector(solution.state).data)
                )
                output.append(solution_vector)
    ...
    def solve(self, rcond=None, mode="default", matrix_columns=None):
        ...
        if self.sparse:
            if matrix_columns is not None:
                raise ValueError(
                    "matrix column selection not implemented for sparse matrices"
                )
            xs, ys, vals = self.get_A_sparse()
            if mode in self.quantum_solvers:
                raise ValueError(
                    "Quantum solver not implemented yet for sparse matrices"
                )
            ...
            if mode in self.quantum_solvers:
                ...
                if mode == "vqls":
                    _invert = self._invert_vqls
                elif mode == "vqls_runtime":
                    _invert = self._invert_vqls_runtime
                elif mode == "qubols":
                    _invert = self._invert_qubols
                x = _invert(A, y, rcond)
...
class LogProductSolver:
    ...
    def solve(self, rcond=None, mode="default", baseline_pairs=None):
        ...
        sol_amp = self.ls_amp.solve(
            rcond=rcond, mode=mode, matrix_columns=baseline_pairs
        )
